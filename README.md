...
- **自定义按键**：新建任意数量的按键（按键 / 摇杆），可选手形（圆形/矩形）
  - 按键行为二合一：**轻点 = 单发，长按 = 连发**（开火键通用：连狙轻点单发、步枪按住连发）
  - 编辑模式拖拽摆放位置，右下角把手调整大小（矩形可独立拉宽/拉高，适配细长条按键）
  - 长按进入「选点模式」点击原神中对应按键的位置完成映射
- **两层可视化**：编辑模式下，游戏层目标位置显示同形状的橙色虚线框标记（带「目标」标签），一眼看清「虚拟按键 -> 游戏按键」的对应关系；未映射的按键会显示「未映射」红字提示
...
- **异形屏适配**：窗口坐标基于物理屏幕（FLAG_LAYOUT_IN_SCREEN），悬浮球避开挖孔区域
